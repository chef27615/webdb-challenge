...
1. Explain the difference between `RDBMS` and `SQL`.
  --`sql` is the query language we are using to get the in from nation from database
  --`RDBMS` is the relationship management system which holding the database

1. Why do tables need a `primary key`?
  -- `primary key` will id each row and/record in a database table

1. What is the name given to a table column that references the primary key on another table.
  -- `foreign key`

1. What do we need in order to have a _many to many_ relationship between two tables.
  -- we need the third table to hold foreign keys and link the relationship together.
